...
void TrackerManager::ReplaceTracker(cv::Mat & frame, const cv::Rect & positionBox, size_t ID)
{
	//LOG(INFO) << " - replace tracker ID: " << ID;
    std::shared_ptr<KCFTracker> tracker_ptr(new KCFTracker(HOG_, FIXEDWINDOW_, MULTISCALE_, LAB_));
	tracker_ptr->init(positionBox, frame);
	traker_map_[ID] = tracker_ptr;
    head_data_map_[ID]->Box() = positionBox;
}
...
bool TrackerManager::IsOutBoard(const cv::Point& cur_pt)
{
    int pixel = bound_mask_.at<uchar>(cur_pt);
    return pixel == 0;
}
